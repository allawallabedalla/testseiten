# Rechnen KI-Firmen selbst mit dem Platzen der Blase?

> Ergebnisoffene Prüfung der These, dass die KI-Industrie durch ihr eigenes
> Verhalten (überstürzte Börsengänge, Insider-Signale, Finanzierungsstrukturen)
> signalisiert, intern mit einer Korrektur oder einem Crash zu rechnen.

- **Status:** ✅ erster Meilenstein abgeschlossen
- **Angelegt am:** 2026-07-14
- **Schlagwörter:** #ki #industrie #blase #boersengang #kapitalmarkt #capex #hype

## 📄 Ergebnisse

- **Bericht:** [`entwuerfe/bericht-ki-blase.md`](entwuerfe/bericht-ki-blase.md) ·
  [PDF](entwuerfe/bericht-ki-blase.pdf) — ergebnisoffene Prüfung (~4.800 Wörter).
- **Methodik- & Evidenz-Anhang:** [`daten/evidenz-methodik-appendix.md`](daten/evidenz-methodik-appendix.md)
  — 62 datierte Rohbefunde mit Stance/Konfidenz + Faktencheck-Protokoll.

## Kernbefund (kalibriertes Urteil, Konfidenz: mittel)

Die These in der **starken Lesart** („Firmen antizipieren intern einen Crash und
positionieren sich defensiv") ist **überwiegend nicht gestützt**; in der
**schwachen Lesart** („Akteure sehen Überhitzung und realisieren Buchgewinne,
solange das Fenster offen ist") **teilweise gestützt**.

- **Dafür:** Führende CEOs (Altman, Zuckerberg, Pichai, Hassabis, Amodei) räumen
  Aug–Nov 2025 wörtlich eine „Blase"/Irrationalität ein; breites Cash-out
  peripherer/bereits börsennotierter Akteure (CoreWeave-Gründer, Magnetar,
  Chip-Insider) bei Höchstbewertungen; punktuelle Bilanz-Vorsicht.
- **Dagegen (stärker gewichtet — revealed preferences):** Rekord-Capex (~725 Mrd.
  USD 2026, +77 %), mehrjährige nicht-stornierbare Compute-Zusagen, Kernlab-Insider
  (Anthropic) *halten* statt zu verkaufen. Wer einen nahen Crash einpreist, bindet
  keine hunderte Milliarden in abschreibungsintensive Assets.
- **Zentrale Spannung:** Worte („Blase") vs. Taten (Doubling-down). Das scheinbar
  stärkste Handlungssignal — Insiderverkäufe — ist dreifach konfundiert (10b5-1,
  Peripherie-Zuordnung, Base-Rate der Aktienvergütung).

## Fragestellung

Kernhypothese (des Fragestellers): **„KI-Firmen rechnen selbst mit einem
Platzen der Blase"**, begründet u. a. mit gedrängten Börsengängen und
weiteren Signalen.

Präzisierte Leitfragen:

- Deuten **Börsengänge/Sekundärverkäufe** 2024–2026 auf ein „Cash-out solange
  die Bewertungen hoch sind" hin?
- Sagen **Führungskräfte** selbst „Blase" – und was tun sie (Insiderverkäufe)?
- Wirken die **Finanzierungsstrukturen** (circular/Vendor-Financing) wie ein
  künstliches Aufblähen der Nachfrage?
- Wie passt der **Rekord-Capex** dazu, der eher gegen einen erwarteten
  baldigen Crash spricht?
- Getrennt davon: Liegt **objektiv** eine Blase vor (Kontextfrage)?

## Hintergrund / Motivation

Ab 2023/24 hat die KI-Branche eine beispiellose Kapital- und Investitionswelle
erlebt. Die These fragt nicht primär, *ob* es eine Blase ist, sondern ob die
Akteure durch ihr eigenes Verhalten verraten, dass sie mit dem Platzen rechnen
(„revealed preferences"). Das ist eine schärfere, prüfbare Frage als das
allgemeine Blasen-Gerede.

> ⚠️ **Hinweis:** Recherchesammlung, keine Anlageberatung. Aussagen über
> aktuelle Ereignisse/Zitate sind quellennah recherchiert und vor Verwendung
> zu verifizieren.

## Vorgehen

Multi-Agent-Recherche (Live-Websuche 2024–2026) mit ergebnisoffenem Test:
Kapitalmarkt/IPOs, Lab-Finanzierung, Capex-Boom, Führungsaussagen/Insider,
Skeptiker/ROI, sowie ein ausdrücklicher Bull-Case (Gegenposition). Danach
Synthese zu einem kalibrierten Urteil und adversarialer Faktencheck.

## Erkenntnisse

_(wird nach Abschluss der Pipeline mit dem Bericht befüllt)_

## Offene Fragen / Nächste Schritte

- [ ] Bericht + Evidenz-Anhang aus der Pipeline ablegen.
- [ ] Zentrale Zahlen/Zitate gegen Primärquellen verifizieren.
